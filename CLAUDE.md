...
- **News** (`data.news.scheduler`): startup full ingest + tier-1 (MAG7 + top ETFs) every 5 min + tier-2 (rest) every 20 min. Peer sources merged every cycle: Massive (paid — summary/image/ticker tags) + Google News RSS (free, near-real-time breadth); Finnhub (free tier) last resort for empty tickers. FinBERT scores each headline per ticker. `?fresh=true` on `/api/news/hot` + `/api/news/{ticker}` overlays a live Google pull (used on News-page open, ticker search, and stock news pages). See `docs/news-module.md`.
...
| Massive — news | Hot-ticker news: summary + image + ticker tags + sentiment | `data/news/ingest.py` → `news_articles` hypertable | ✅ live (peer, every cycle) |
| Google News RSS | News peer + live `?fresh` overlay: free, no key, near-real-time (no summary/image) | `data/news/ingest.py`, `backend/app/api/routes/news.py` | ✅ live (peer, every cycle) |

...
export function MarketStatusBadge({ className }: { className?: string }) {
  const { isOpen, sessionPhase, feedStale, nextOpen } = useMarketStatus();
  // When the session is open per the calendar but the live feed has stalled,
  // never claim "LIVE" — show "DELAYED" in amber so a dead feed isn't disguised
  // as fresh data.
  const label = feedStale ? "DELAYED" : PHASE_LABEL[sessionPhase];
  ...
  const pill = (
    <span
      ...
      title={
        feedStale
          ? "US market open, but the live feed has stalled — prices may be delayed"
          : isOpen
            ? `US market: ${label}`
            : reopen

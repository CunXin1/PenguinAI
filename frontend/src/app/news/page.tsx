...
const SENT: Record<NewsArticle["sentiment"], { label: string; chip: string }> = {
  positive: { label: "Bullish", chip: "text-emerald-400 bg-emerald-500/10 border-emerald-500/30" },
  negative: { label: "Bearish", chip: "text-red-400 bg-red-500/10 border-red-500/30" },
  neutral: { label: "Neutral", chip: "text-zinc-400 bg-zinc-700/40 border-zinc-600/50" },
};
    ...
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-5">
      <div>
        <h1 className="text-xl font-bold text-white flex items-center gap-2">
          <Newspaper size={20} className="text-sky-400" /> Market News
        </h1>
      ...
      <Card className="p-4">
        <div className="flex items-center justify-between mb-2">
          <p className="text-sm font-semibold text-zinc-200">Today&apos;s news sentiment</p>
          <span className={cn("text-sm font-semibold", net === "Bullish" ? "text-emerald-400" : "text-red-400")}>{net} bias</span>
        </div>
        <div className="h-2 rounded-full overflow-hidden flex bg-zinc-800">
          ...
          <div className="bg-zinc-600" style={{ width: `${(counts.neutral / total) * 100}%` }} />
          <div className="bg-red-500" style={{ width: `${(counts.negative / total) * 100}%` }} />
        </div>
        <div className="flex gap-4 mt-2 text-xs text-zinc-500">
          <span><span className="text-emerald-400 font-mono">{counts.positive}</span> bullish</span>
          <span><span className="text-zinc-300 font-mono">{counts.neutral}</span> neutral</span>
          <span><span className="text-red-400 font-mono">{counts.negative}</span> bearish</span>
        </div>
      </Card>
        ...
        <Link href={`/news/${featured.id}`} className="block group">
          <Card className="p-5 hover:border-zinc-700 transition-colors">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs text-zinc-600">Featured · {featured.source}</span>
            </div>
            <h2 className="text-lg font-bold text-zinc-100 leading-snug group-hover:text-white transition-colors">{featured.headline}</h2>
            <p className="text-sm text-zinc-400 mt-2 leading-relaxed">{featured.summary}</p>
            <span className="inline-flex items-center gap-1 text-sm text-sky-400 mt-3">
        ...
        {FILTERS.map((x) => (
          <button
            ...
            className={cn(
              ...
              f === x ? "bg-zinc-800 text-white" : "text-zinc-500 hover:text-zinc-300"
            )}
          >
      ...
      <div className="space-y-3">
              ...
              <Card className="p-4 hover:border-zinc-700 transition-colors">
                <div className="flex items-start justify-between gap-4">
                  <div className="space-y-1.5 min-w-0">
                    <h2 className="text-base font-semibold text-zinc-100 leading-snug group-hover:text-white transition-colors">{n.headline}</h2>
                    <p className="text-sm text-zinc-400 leading-relaxed line-clamp-2">{n.summary}</p>
        ...
        {feed.length === 0 && <p className="text-center text-sm text-zinc-600 py-8">No {f} headlines right now.</p>}
      </div>
    </div>
  );
}

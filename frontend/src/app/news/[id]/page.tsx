...
const SENT: Record<NewsArticle["sentiment"], { label: string; cls: string }> = {
  positive: { label: "Bullish", cls: "text-emerald-400 bg-emerald-500/10 border-emerald-500/30" },
  negative: { label: "Bearish", cls: "text-red-400 bg-red-500/10 border-red-500/30" },
  neutral: { label: "Neutral", cls: "text-zinc-400 bg-zinc-700/40 border-zinc-600/50" },
};
...
export default async function NewsArticlePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-5">
      <Link href="/news" className="text-sm text-zinc-500 hover:text-zinc-300 flex items-center gap-1 w-fit transition-colors">
      ...
      <div className="space-y-3">
        ...
        <h1 className="text-2xl font-bold text-white leading-tight">{article.headline}</h1>
      ...
      {related.length > 0 && (
        <Card className="p-4">
          <p className="text-sm font-semibold text-zinc-200 mb-3">Related signals</p>
          <div className="space-y-1">
            {related.map((u) => (
              <Link
                key={u.ticker}
                href={`/signals/${u.ticker}`}
                className="flex items-center justify-between py-2 px-2 -mx-2 rounded-md hover:bg-zinc-800/40 transition-colors"
              >
                <div className="flex items-center gap-3 min-w-0">
                  <span className="font-mono font-semibold text-sm text-zinc-100 w-14 shrink-0">{u.ticker}</span>
      ...
      <p className="text-xs text-zinc-600 pt-3 border-t border-zinc-800/60">

      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-white flex items-center gap-2">
            <Star size={20} className="text-amber-400" /> Watchlist
          </h1>
          <p className="text-sm text-zinc-500 mt-0.5">{ready ? tickers.length : 0} tracked · saved locally</p>
        </div>
        <form
          onSubmit={add}
          className="flex items-center gap-2 rounded-lg bg-zinc-900 border border-zinc-800 px-3 py-2 focus-within:border-sky-500/60 transition-colors"
        >
          ...
          <input
            ...
            className="bg-transparent outline-none text-sm text-zinc-200 placeholder-zinc-600 w-28"
          />
          <button type="submit" className="text-sky-400 hover:text-sky-300 transition-colors" aria-label="Add">
            <Plus size={16} />
          </button>
        </form>
      </div>
      ...
      {!ready ? (
        ...
        <Card className="p-10 text-center">
          ...
          <p className="text-xs text-zinc-600 mt-1">
            ...
            <Link href="/screener" className="text-sky-400 hover:text-sky-300">
              browse the screener
            </Link>
            .
          </p>
        ...
        <div className="space-y-2">
          {rows.map((r) => {
            ...
            return (
              <Card key={r.ticker} className="p-3 flex items-center gap-3 hover:border-zinc-700 transition-colors">
                <Link href={`/signals/${r.ticker}`} className="flex items-center gap-3 flex-1 min-w-0">
                  <div className="w-16 shrink-0">
                    <p className="font-mono font-semibold text-sm text-zinc-100">{r.ticker}</p>
                  ...
                  <div className="text-right w-20 shrink-0">
                    <p className="font-mono text-sm text-zinc-200">{r.price != null ? money(r.price) : "—"}</p>
                    {r.change_pct != null && (
                      <p className={`font-mono text-xs ${up ? "text-emerald-400" : "text-red-400"}`}>{signedPct(r.change_pct, 1)}</p>
                    )}
                  </div>
